...
TEST(instance_reader, read_instance_1_batch)
{
...
TEST(instance_reader, read_instance_2_batch)
{
	std::istringstream input(instance_text());
	...
	EXPECT_EQ(instance.batchs()[0].jobs().size(), 2);
	EXPECT_EQ(instance.batchs()[1].jobs().size(), 2);
