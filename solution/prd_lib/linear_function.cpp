...
namespace solver
{
	...
	Tournee Tournee::create_tournee(const std::vector<int>& vect)
	{
		Tournee ret;
		ret.nb = vect.size();
	...
	{
		if (nb <= 0) {
			throw std::exception("ERREUR: OQNAER");
		}
		nb--;
	}
	...
	Tournee Tournee::create_tournee_nearest_insertion(const instance& inst, const vector<int>& vect)
	{
		...
		vector<std::pair<int, int>> ind_dist(ret.nb);
		for (int i = 0; i < ret.nb; i++)
		{
		...
		for (int i = 0; i < ret.nb; i++)
		{
			...
			{
				ind_dist[j].second = inst.get_distance_between(last_index, ind_dist[j].first);
				k = j;
				auto tmp = ind_dist[j];
	...
	int Tournee::eval_routing_cost(const instance& inst) const
	{
		...
		{
			index = tab_c[i];
			routing_cost += inst.get_distance_between(old_index, index);
			old_index = index;
		}
	...
	Fct_lin Fct_lin::create_fct_lin(const Tournee& road, instance const & inst, int m_a, int m_b)
	{
		Fct_lin fct;
	...
	Fct_lin Fct_lin::create_fct_max(int min_a, int min_b)
	{
		...
		l.alpha = 0;
		l.t = min_a;
		l.c = std::numeric_limits<int>::max();
		l.tournee = Tournee();
		l.tournee.tab_c = new int[0];
		l.tournee.nb = 0;

		fct.tab_link.push_back(l);
		fct.eval_max_b = std::numeric_limits<int>::max();
		return fct;
	}
	...
	{
		if (t < min_a || t > max_b) {
			throw std::exception("ERREUR : cade ALSUF");
		}
	...
	{
		if (t < min_a || t > max_b) {
			throw std::exception("ERREUR : cade ODISX");
		}
	...
	{
		if (t < min_a || t > max_b) {
			throw std::exception("ERREUR : cade AUDSVSRF");
		}
	...
	int Fct_lin::calcul_aire_sous_la_courbe(int new_min_a, int new_min_b) const
	{
		...
		if (max_b == std::numeric_limits<int>::min()) {
			m_a = new_min_a;
			m_b = new_min_b;

			if (m_a < min_a || m_a > max_b) {
				std::cout << "ISBEF" << std::endl;
			}
	...
	Fct_lin Fct_lin::minimum_fct(const Fct_lin& f1, const Fct_lin& f2) {
		...
		if (f1.min_a != f2.min_a || f2.max_b != f2.max_b) {
			std::cout << "ERREUR : code ICMSTO" << std::endl;
		}
		...
		function.eval_max_b = std::min(f1.eval_max_b, f2.eval_max_b);
	...
	pair<int, int> Fct_lin::a_b_inf(instance& inst, const vector<int>& batch_old)
	{
		...
		{
			distance = inst.get_distance_between(inst.get_job_count(), batch_old[i]);
			...
			{
				distance = inst.get_distance_between(batch_old[i], batch_old[j]);
				if (distance > max_dist) {
					max_dist = distance;
				}
			}
	...
	Fct_lin Fct_lin::generate_pure_random_fct(instance const & inst, const vector<int>& batch_old, int min_a, int max_b)
	{
	...
	Fct_lin Fct_lin::generate_fct_with_local_search(instance const  & inst, Tournee& tournee_init, int min_a, int max_b)
	{
		////////
	...
	Fct_lin Fct_lin::generate_fct_with_four_extrema(instance const & inst, Tournee & tournee, int min_a, int max_b)
	{
