...
namespace solver
{
	...
	Covering_tree::Covering_tree(instance const & inst, const vector<int>& batch_old)
		:Covering_tree{ inst }
	{
		edges = vector<Edge>();
	...
	void Covering_tree::connect_ss_ens(vector<vector<int>>& sous_ens) {
		...
		while (sous_ens.size() != 1) {
			...
			////
			//recup?ration de larrete de poid minimum
			...
			for (int ss_1 = 0; ss_1 < sous_ens.size(); ss_1++)
			{
				for (int ss_2 = ss_1 + 1; ss_2 < sous_ens.size(); ss_2++)
				{
					for (int ind_1 = 0; ind_1 < sous_ens[ss_1].size(); ind_1++)
					{
						...
						{
							cost = inst.get_distance_between(sous_ens[ss_1][ind_1], sous_ens[ss_2][ind_2]);
			...
			edges.emplace_back(sous_ens[b_ss1][b_ind_1], sous_ens[b_ss2][b_ind_2], best_cost);

			////
			// mise ? jour des links with
	...
	Branch_and_bound::Branch_and_bound(instance const & instance)
		:inst{ instance }
	{
	}
	...
	Fct_lin Branch_and_bound::generate_fct_with_branch_and_bound(const vector<int>& batch_old, int min_a, int max_b)
	{
		...
		//cout de la solution en fonction de la date de d?part
		//? un ?tat d'avancement du B&B => LowerBound
		...
		int branch_node, Vec_date_cost tab_date_cost, int time_travel_for_last_job)
	{
		if (nb_resting_job > 1) { //si pas une feuille
			...
			int dist = inst.get_distance_between(last_visited_site_index, branch_node);
			time_travel_for_last_job += dist;
			// tournee incr?ment?e dans la branch precedente
			//*****		

			//*****
			//mise ? jour du tableau de cout pour toute les date de d?part encore valide
			for (int date = tab_date_cost.min_a; date <= tab_date_cost.max_b; date++)
			{
				if (tab_date_cost.date_is_valide(date)) {
					tab_date_cost.add_cost(date, dist /* cout de routing == distance */
						+ (std::max(0, date + time_travel_for_last_job - job.get_due_date())*job.get_penalty_per_delivery_delay())); // cout de retard du job
				}
			}
			...
			//On checke si la lower bound et inf?rieure ? la upper bound pour au moins une date
			for (int date = tab_date_cost.min_a; date <= tab_date_cost.max_b; date++)
			{
	...
	Branch_and_bound::Vec_date_cost Branch_and_bound::min_assigment(Covering_tree& covering_tree, Branch_and_bound::Vec_date_cost& tab_date_cost, int travel_time_to_last_job, int branch_node, const vector<vector<int>>& test)
	{
		...
		int i_job = 0;
		//pour chacun des jobs restant ? assigner
		//on calcul ses p?nalit?s de retard associ? si le job
		//et livr? en i^eme position
		//==> 1 ligne: 1 job
		//==> et pour chaque colonne: le retard du job associ? ? cette position dans la livraison
		for (int index_job : job_resting_for_assignement)
		{
			if (index_job != branch_node) {
				...
				{
					// pr?sence de n?gatif, mais qui compte comme 0 dans l'algo hongrois
					tab_job_assignement_initial[i_job][i] = (delivery_date[i] - job.get_due_date())*job.get_penalty_per_delivery_delay();
				}
				i_job++;
			}
		}

		// ==> cette premi?re initialisation se fait en min_a
		//FIN INITIALISATION
		//*******
		...
		for (int date = tab_date_cost.min_a; date <= tab_date_cost.max_b; date++)
		{
			if (tab_date_cost.date_is_valide(date)) {
				...
				//il n'est pas n?c?ssaire d'augmenter les lignes de l'algo hongrois
				//pour lequelle le job est en retard ? tous les postes de livraison
				//(et donc en retard pour le premier)

				//===> Conclusion: si tout les jobs sont en retard, pas besoin de mettre ? jour
				//la matrice et donc de r?apliqu? l'algo hongrois

				//l'affectation reste la m?me si tous les jobs sont en retard, il suffi juste de les appliquer ? la nouvelle
				//matrice initial (dont les co?t continue de grimper)
				if (reste_job_en_avance) {
					...
					if (date != old_valide_date) {
						...
						for (int index_job : job_resting_for_assignement)
						{
							//il n'est pas n?c?ssaire d'augmenter les lignes de l'algo hongrois
							//pour lequelle le job est en retard ? tous les postes de livraison
							//(et donc en retard (ou juste ? l'heure) pour le premier)
							//d'apr?s le tableau d'assignement initial
							if (index_job != branch_node) {
					...
					//*****
					//DEBUT ALGO HONGROIS 
					//Note : consid?rer les vals n?gatives comme des z?ros 
					//repr?sente des changement minimie
					////
					//Etape 0 - Etape de r?duction
					////

					//Pour chaque ligne de la matrice, on retire ? l'ensemble de la ligne la valeur minimale de celle-ci. La matrice a alors au moins un z?ro par ligne. On r?p?te la m?me op?ration sur les colonnes. 
					//On obtient alors un probl?me ?quivalent avec une matrice ayant au moins un z?ro par ligne et par colonne. (WIKIPEDIA)

					//==> pour les lignes
					//on sait que le plus petit element d'une ligne est son premier element
					//(correspond au cout associer si le job est livr? en premier
					int cout_min;
					for (int i = 0; i < nb_livraison; i++)
					{
				 ...
				 //? chaque fois que la date avance, 
				 //les couts associ?s dans la matrice initial augmentent
				i_job = 0;
				for (int index_job : job_resting_for_assignement)
				{
					if (index_job != branch_node) {
				...
				//*****
				//RECUPERATION DU COUT ASSOCI? A CETTE DATE
				min_affect_a_date = 0;
				for (int i = 0; i < tab_job_assignement_initial.size(); i++)
				{
					//compte uniquement des valeurs positives, z?ro sinon
					min_affect_a_date += std::max(0, tab_job_assignement_initial[i][zero_couvrant[i]]);
				}


				//incr?mentation du tab de renvoie swarzy
				//(initialis? a -1 pour chaque date)
	...
	void Branch_and_bound::aux_0_couvrant_non_couvrant(const vector<vector<int>>& matrix, vector<int>& zero_couvrant, vector<vector<int>>& zero_barre)
	{
		...
		for (int i = 0; i < taille; i++) {
			for (int j = 0; j < taille; j++) {
				//les valeurs n?gative compte comme des 0
				if (matrix[i][j] <= 0) {
		...
		{
			nb_min_zero = std::numeric_limits<int>::max();
			...
			while (index < taille && nb_min_zero != 1)
			{
				if (zero_ligne[index].size() != 0 && zero_ligne[index].size() < nb_min_zero) {
					...
					ind_colonne_du_zero = zero_ligne[index][0]; //on consid?re que l'on enl?ve le premier 0 de la ligne / de la colonne
				} index++;
			}
			...
			while (index < taille && nb_min_zero != 1)
			{
				if (zero_colonne[index].size() != 0 && zero_colonne[index].size() < nb_min_zero) {
					...
					ind_ligne_du_zero = zero_colonne[index][0]; //on consid?re que l'on enl?ve le premier 0 de la ligne / de la colonne
				} index++;
			}

			//remplissage de zero couvrant et zero barr?
	...
	void Branch_and_bound::aux_ligne_marquee_colonne_barree(vector<int>& zero_couvrant, vector<vector<int>>& zero_barre, vector<bool>& ligne_marque, vector<bool>& colonne_marque)
	{
		...
		while (new_ligne_marque) {
			...
			int cpt_c;
			//on barre les colonne avec des zero barr?s sur les ligne marque
			for (int j = 0; j < taille; j++)
			{
				if (!colonne_marque[j]) {
					for (int i = 0; i < taille; i++)
					{
						if (ligne_marque[i]) {
							cpt_c = 0;
							while (cpt_c < zero_barre[i].size()) {//&& zero_barre[i][cpt_c] <= j FAUX pas forcement barr? dans l'ordre
	...
	void Branch_and_bound::aux_approfondisement_allocation_de_ligne(vector<vector<int>>& zero_barre, vector<int>& zero_couvrant, vector<int>& zero_couvrant_col)
	{
		...
		{
			if (zero_couvrant[ligne_non_couv] == -1) {//cette ligne est belle est bien non allou?e

				for (int i = 0; i < taille; i++) //mise ? nulle des possibilit? d'acc?s
				{
	...
	int Branch_and_bound::rec_approfondissement_de_ligne(int index_ligne, vector<vector<int>>& zero_barre, vector<int>& zero_couvrant, vector<int>& zero_couvrant_col, vector<int>& colonne_i_acces_via_lin)
	{
		...
		while (i < zero_barre[index_ligne].size() && ind_colonne_libre == -1)
		{
			...
			if (colonne_i_acces_via_lin[ind_col] == -1) { //si la colonne n'est pas d?j? acced?
				...
				if (zero_couvrant_col[ind_col] == -1) {
					//cette colonne n'est pas couverte, on peut ?tablir une correspondance ligne colonne non couvert
